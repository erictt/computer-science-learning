...
def generate_models(x, y, degs):
    ...
    models = []
    for degree in degs:
        models.append(np.asarray(np.polyfit(x, y, degree)))
    return models
...
def r_squared(y, estimated):
    ...
    return 1 - sum([(i - j)**2 for i,j in zip(y, estimated)]) / sum([(i - mean)**2 for i in y])
        
print(r_squared([1,2,3], [2,3,4]))
...
def evaluate_models_on_training(x, y, models):
    ...
    pylab.title("Daily temperatures between 1961 to 2015")
    for model in models:
        estimatedYs = pylab.polyval(model, x)
        pylab.plot(x, estimatedYs, 
                   label = "R**2 = " + str(round(r_squared(y, estimatedYs), 5)))
        pylab.legend(loc="best")
