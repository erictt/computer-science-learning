...
* ![](../.gitbook/assets/15246609225677%20%281%29.jpg)
...
* Given \\(a, x \in \mathbb {R}^ m\\), we can use \\(P _a\( x \)\\) and \\(P_ a ^{\perp}\( x \)\\) to represent the projection of vector \\(x\\) onto \\({\rm Span}\({ a} \)\\) and \\({\rm Span}\({ a} \)^{\perp}\\).
...
* Definition: Let \\(q_0, q\_1, \ldots, q_{k-1} \in \mathbb{R}^m\\). Then these vectors are \(mutally\) orthonormal if for all \\(0 \le i,j &lt; k\\): \\[q\_i^T q\_j = \begin{cases} 1 & \text{ if } i = j \ 0 & \text{ otherwise. }\end{cases}\\]
  * \\(\lVert q\_i \rVert\_2 = 1\\)
  * \\(q_i\\) is orthogonal to \\({q\_0, q\_1, \ldots, q_{i-1}, q_{i+1}, \ldots, q_{m}}\\).
...
* Starting with linearly independent vectors \\(a_0, a\_1, \ldots, a_{n-1} \in \mathbb{R}^m\\), the following algorithm computes the mutually orthonormal vectors \\(q_0, q\_1, \ldots, q_{n-1} \in \mathbb{R}^m\\) such that \\(\text{Span}\({a_0, a\_1, \ldots, a_{n-1}}\) = \text{Span}\({q_0, q\_1, \ldots, q_{n-1}}\)\\):
* ![](../.gitbook/assets/15251022826860.jpg)
* ![](../.gitbook/assets/15397008238987.jpg)
* The algorithm:
  * ![](../.gitbook/assets/15248052802711%20%281%29.jpg)
...
* If one partitions
* ![](../.gitbook/assets/15248055719429%20%281%29.jpg)
* then 
* ![](../.gitbook/assets/15248055975514%20%281%29.jpg)
...
* An algorithm for computing the QR factorization is given by
  * ![](../.gitbook/assets/15248058147454.jpg)

#### Change of Basis

* A vector \\(b \in \mathbb{R}^m\\) and a matrix \\(Q \in \mathbb{R}^{m \times n}\\) with mutually orthonormal columns.
* \\(Q^T Q = Q Q^T = Q^{-1} Q = Q Q^{-1} = I\\)
* \\(b = Q Q^T b = \left\(\begin{array}{c\|c\|c}q_0 & q\_1 & \cdots & q_{n-1}\end{array}\right\)\left\(\begin{array}{c}q_0^T \ q\_1^T \ \vdots \ q_{n-1}^T\end{array}\right\) b = q_0^T b q\_0 + q\_1^T b q\_1 + \ldots + q_{i-1}^T b q\_{i-1}\\)
  * \\(q\_0^T b q\_0 = q\_0 q\_0^T b\\) because \\(q\_0^T b \\) is scalar.
...
* Any matrix \\(A \in \mathbb{R}^{m \times n}\\) can be written as the product of three matrices, the Singular Value Decomposition \(SVD\): \\[A = U \Sigma V^T\\] where
  ...
  * \\(\Sigma \in \mathbb{R}^{r \times r}\\) is a diagonal matrix with positive diagonal elements that are ordered so that \\(\sigma_{0,0} \ge \sigma_{1,1} \ge \ldots \ge \sigma\_{\(r-1\),\(r-1\)} &gt; 0\\).
  * \\(V \in \mathbb{R}^{n \times r}\\) and \\(V^T V = I\\) \(V has orthonormal columns\).
  * \\(r\\) equals the rank of matrix \\(A\\).
* If we partition
  * ![](../.gitbook/assets/15248058876303%20%281%29.jpg)
* where \\(U_L\\) and \\(V\_L\\) have \\(k\\) columns and \\(\Sigma_{TL}\\) is \\(k \times k\\), then \\(U_L \Sigma_{TL} V_L^T\\) is the “best” rank-k approximation to matrix B. So, the “best” rank-k approximation \\(B = AW^T\\) is given by the choices \\(A = U\_L\\) and \\(W = \Sigma_{TL} V\_L\\).
