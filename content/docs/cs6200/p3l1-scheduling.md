...
* CFS uses a [read-black tree](/algorithms-1/week-5/#red-black-bsts) as the runqueue data structure.
...
* The Completely Fair Scheduler (CFS) of Linux
  ...
  * Several main configs that the scheduler used:
    ...
    * **weighting(niceness)**: enable control over process priority. CFS maps the **nice** value of each process to a weight, as shown here:
      ...
      * To improve the efficiency, the scheduler uses [Red-Black Trees](https://cs.ericyy.me/algorithms-1/week-5/#red-black-bsts) with the **vruntime** values of each process and run the **min-vrumtime** process at each time and push it back to the tree after the time slice.
